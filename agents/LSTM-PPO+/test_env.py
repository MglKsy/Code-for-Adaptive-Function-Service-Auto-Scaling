...
#主要用于管理 Deployments、DaemonSets、StatefulSets 等应用的 扩缩容、更新、删除。
#在 OpenFaaS 场景下，它可以用来调整函数的副本数（scaling）。
...
class Environment(gym.Env):
    ...
    def _initial_setup(self):
        ...
        self.func_cpu = 100  # millicores
        self.func_mem = round((256 / 1024), 2)  # in GBi

        # custom metrics from env
        logdir = f"eval_logs/{datetime.now().strftime('%Y%m%d-%H%M%S')}/{model_name}"
